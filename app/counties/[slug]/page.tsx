...
export default function CountyDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div classNameName="min-h-screen flex flex-col">
      ...
      <main classNameName="flex-1 bg-white">
        {/* County Header */}
        <div classNameName="bg-gray-50 py-8">
          <div classNameName="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div classNameName="flex items-center space-x-4">
              <div classNameName="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center">
                <span classNameName="text-xl font-bold text-gray-600">{county.code}</span>
              </div>
              <h1 classNameName="text-3xl font-bold text-gray-900">{county.code} - {county.name}</h1>
            </div>
          </div>
        </div>

        {/* Hero Image */}
        <div classNameName="w-full h-96 bg-gradient-to-br from-gray-200 to-gray-300 relative">
          <div classNameName="absolute inset-0 flex items-center justify-center">
            <span classNameName="text-gray-400 text-lg">County Image Placeholder</span>
          </div>
        </div>

        {/* Info Section */}
        <div classNameName="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div classNameName="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
            <div>
              <h2 classNameName="text-2xl font-bold text-gray-900 mb-4">TOTAL SERVICES</h2>
              <p classNameName="text-6xl font-bold text-ecitizen-green">{county.services}</p>
            </div>
            <div>
              <h2 classNameName="text-2xl font-bold text-gray-900 mb-4">OVERVIEW</h2>
              <p classNameName="text-gray-700 leading-relaxed">{county.overview}</p>
            </div>
          </div>
          ...
          <div classNameName="mb-8">
            <div classNameName="flex items-center justify-between mb-6">
              <h2 classNameName="text-2xl font-bold text-gray-900">SERVICES</h2>
              <div classNameName="flex-1 max-w-md ml-4">
                <div classNameName="relative">
                  <input
                    type="text"
                    placeholder="Search services"
                    classNameName="w-full px-4 py-2 pl-10 border border-gray-300 rounded-md focus:ring-2 focus:ring-ecitizen-green focus:border-transparent outline-none"
                  />
                  <svg classNameName="w-5 h-5 text-gray-400 absolute left-3 top-1/2 transform -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div classNameName="space-y-3">
              {services.map((service, index) => (
                <div
                  key={index}
                  classNameName="bg-gray-50 border border-gray-200 rounded-lg p-4 hover:bg-gray-100 hover:border-ecitizen-green transition cursor-pointer flex items-center justify-between"
                >
                  <span classNameName="text-gray-900 font-medium">{service}</span>
                  <svg classNameName="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
